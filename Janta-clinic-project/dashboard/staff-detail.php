...
<body>
    ...
    $offset = ($page - 1) * $limit;
    $staffpage = $staffmanager->getAllStaffPagination($offset, $limit);
  ...
  ?>
    <main>
        <div>
            <h1
                ...
                Registered Staff's</h1>
            <div class="mb-5"> <a type="button" href="../dashboard/staff-register.php"
    ...
    <div class="flex">
        <input type="search" id="search-dropdown" name="id"
            ...
            placeholder="Search by staff firstname or Id..." required />
        <button type="submit"
            ...
            <?php if (isset($_POST['id'])) {
                $id = $_POST['id'];
                $staffSearch = $staffmanager->getstaffSearch($id);
            ...
            ?>
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mb-8">
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <?php foreach ($staffSearch as $staff): ?>
                                    <tr
                                        class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                        <th scope="row"
                                            class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            <?php echo htmlspecialchars($staff->getStaffId()); ?>
                                        </th>
                                        <td class="px-6 py-4">
                                            <?php echo htmlspecialchars($staff->getFullName()); ?>
                                        </td>
                                        <td class="px-6 py-4">
                                            <?php echo htmlspecialchars($staff->getStaffRole()); ?>
                                        </td>
                                        <td class="px-6 py-4">
                                            <?php echo htmlspecialchars($staff->getStaffPhone()); ?>
                                        </td>
                                        <td class="px-6 py-4">
                                            <?php echo htmlspecialchars($staff->getStaffDate()); ?>
                                        </td>
                                        <td class="px-6 py-4">
                                            <a href="../dashboard/staff-manage.php?id=<?php echo htmlspecialchars($staff->getStaffId()); ?>&page=<?php echo htmlspecialchars($page); ?>"
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <?php foreach ($staffpage as $staff): ?>
                            <tr
                                class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                <th scope="row"
                                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
        ...
        <ul class="inline-flex -space-x-px text-base h-10 mt-10">
            <?php if ($page > 1) { ?>
                <li>
                    <a href="../dashboard/staff-detail.php?page=<?php echo htmlspecialchars($page - 1) ?> ?>"
            ...
            ?>
                <li class="<?php $active ?>">
                    <a href="../dashboard/staff-detail.php?page=<?php echo htmlspecialchars($i); ?>"
